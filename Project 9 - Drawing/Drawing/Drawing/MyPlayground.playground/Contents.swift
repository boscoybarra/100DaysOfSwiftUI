...
//animatableData only with only one value
...
//  We need to add this do that we see how animations take effect
...
struct ContentView: View {
    ...
    var body: some View {
        Trapezoid(insetAmount: insetAmount)
            .frame(width: 200, height: 100)
            .onTapGesture {
//            when adding withAnimation is changes do not make effect we need to add animatableData that takes care of animations behind the scenes
                withAnimation {
                    self.insetAmount = CGFloat.random(in: 10...90)
                }
            }
...
//AnimatablePair we can add multiple data points
